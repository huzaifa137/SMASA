...
<head>
    ...
    <title>Report Card (Minimal) — {{ $exam->exam_name }}</title>
    ...
    @php
        ...
        */
        ...
        // This view IS the Minimal design — no runtime switching needed.
...
<body class="tpl-{{ $template }}">
    ...
    @whole($totObt) / @whole($totMax)
                ...
                @if(($cfg['term_ends_on'] ?? true) || ($cfg['next_term_starts_on'] ?? true))
                    ...
                    @endphp
                    @if(($cfg['term_ends_on'] && $termEndsOnMi) || ($cfg['next_term_starts_on'] && $nextTermStartsOnMi))
