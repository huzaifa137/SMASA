...
@section('css')
    <style>
        ...
        .marks-table tbody tr:hover { background: #faf9ff; }
        ...
        .marks-input.valid   { border-color: #1a7a4a; background: #f0fdf4; }
        .marks-input.invalid { border-color: #c0392b; background: #fef2f2; }
        ...
        .grade-D { background: #d4f5e2; color: #1a7a4a; }
        .grade-C { background: #cfe2ff; color: #0a4191; }
        .grade-P { background: #fff3cd; color: #856404; }
        ...
        .comment-input:focus { border-color: #5351e4; }
    </style>
@endsection
    ...
    <div class="exam-hero mb-4">
        <div class="d-flex justify-content-between align-items-start">
            <div class="flex-grow-1">
                <div class="mb-1" style="font-size:.82rem; color:rgba(255,255,255,.7);">
                    <i class="fas fa-file-alt me-1"></i>{{ $exam->exam_name }} &rsaquo;
                    <strong class="text-white">{{ Helper::recordMdname($classSubject->subject_id) }}</strong>
                </div>
                <h3 class="text-white fw-bold mb-2">Enter Marks</h3>
                <div class="d-flex flex-wrap gap-2">
                    <span style="background:rgba(255,255,255,.15); border-radius:.4rem; padding:.25rem .7rem; font-size:.8rem; color:#fff;">
                        <i class="fas fa-chalkboard-teacher me-1"></i>
                        {{ Helper::recordMdname($classSubject->class_id) }}
                        @if($classSubject->stream_id) — {{ $classSubject->stream_id }} @endif
                    </span>
                    <span style="background:rgba(255,255,255,.15); border-radius:.4rem; padding:.25rem .7rem; font-size:.8rem; color:#fff;">
                        <i class="fas fa-star me-1"></i> Out of {{ $exam->total_marks }}
                    </span>
                    <span style="background:rgba(255,255,255,.15); border-radius:.4rem; padding:.25rem .7rem; font-size:.8rem; color:#fff;">
                        <i class="fas fa-hourglass-end me-1"></i> Deadline: {{ $exam->marks_entry_deadline->format('d M Y') }}
                    </span>
                </div>
                {{-- Progress bar --}}
                @php
                ...
                <div class="d-flex align-items-center gap-2 mt-2">
                    <div class="progress-bar-wrap flex-grow-1" style="max-width:280px;">
                        <div class="progress-bar-fill" style="width:{{ $progress }}%"></div>
                    </div>
                    <span style="font-size:.8rem; color:#fff;">{{ $entered }}/{{ $total }} entered ({{ $progress }}%)</span>
                </div>
            </div>
            <a href="{{ route('examination.marks.entry', $exam->id) }}"
               class="btn btn-light btn-sm fw-semibold ms-3" style="border-radius:.5rem;">
                            ...
                            <tr data-student-id="{{ $student->id }}">
                                ...
                                <td>
                                    <span class="grade-badge grade-cell
                                        @php
                                            $g = $mark?->grade ?? '';
                                            if(str_starts_with($g,'D')) echo 'grade-D';
                                            elseif(str_starts_with($g,'C')) echo 'grade-C';
                                            elseif(str_starts_with($g,'P')) echo 'grade-P';
                                            elseif(str_starts_with($g,'F')) echo 'grade-F';
                                        @endphp"
                                ...
                                <td>
                                    <span class="text-muted remark-cell" style="font-size:.8rem;" id="remark_{{ $student->id }}">
                                        {{ $mark?->grade_remark ?? '—' }}
                                    </span>
                                </td>
                                <td>
                                    <input type="text"
                                           ...
                                           value="{{ $mark?->teacher_comment ?? '' }}"
                                           placeholder="Optional comment">
                                </td>
                                <td>
                                    <span class="badge status-cell
                                        @if($mark?->status === 'entered') bg-success
                                        @elseif($mark?->status === 'verified') bg-primary
                                        @else bg-secondary @endif"
                                        style="font-size:.72rem;" id="status_{{ $student->id }}">
...
<div class="modal fade" id="gradeScaleModal" tabindex="-1">
    <div class="modal-dialog modal-sm">
        <div class="modal-content border-0" style="border-radius:1rem;">
            <div class="modal-header" style="background:linear-gradient(135deg,#2C29CA,#5351e4); border-radius:1rem 1rem 0 0;">
