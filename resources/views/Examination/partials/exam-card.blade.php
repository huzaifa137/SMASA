<div class="exam-card status-{{ $exam->status }}" data-id="{{ $exam->id }}" onclick="viewExamDetails({{ $exam->id }})">
    <div class="d-flex justify-content-between align-items-start mb-2">
        ...
        <div class="dropdown">
            <button class="btn btn-sm" style="background: none;" onclick="event.stopPropagation();" data-bs-toggle="dropdown">
                <i class="fas fa-ellipsis-v"></i>
            </button>
            <ul class="dropdown-menu">
                @if($exam->status === 'draft')
                    <li><a class="dropdown-item" href="#" onclick="event.stopPropagation(); updateExamStatus({{ $exam->id }}, 'active')">
                        <i class="fas fa-play text-success me-2"></i> Activate
                    </a></li>
                    <li><hr class="dropdown-divider"></li>
                    <li><a class="dropdown-item text-danger" href="#" onclick="event.stopPropagation(); deleteExam({{ $exam->id }})">
                        <i class="fas fa-trash me-2"></i> Delete
                    </a></li>
                @elseif($exam->status === 'active')
                    <li><a class="dropdown-item" href="#" onclick="event.stopPropagation(); updateExamStatus({{ $exam->id }}, 'marks_entry')">
                        <i class="fas fa-pen-alt text-warning me-2"></i> Open Marks Entry
                    </a></li>
                    <li><a class="dropdown-item" href="{{ route('examination.marks.entry', $exam->id) }}" onclick="event.stopPropagation();">
                        <i class="fas fa-table text-primary me-2"></i> Enter Marks
                    </a></li>
                @elseif($exam->status === 'marks_entry')
                    <li><a class="dropdown-item" href="#" onclick="event.stopPropagation(); updateExamStatus({{ $exam->id }}, 'closed')">
                ...
                @elseif($exam->status === 'closed')
                    <li><a class="dropdown-item" href="#" onclick="event.stopPropagation(); updateExamStatus({{ $exam->id }}, 'results_released')">
    ...
    <div class="mt-2">
        <div class="progress" style="height: 3px;">
            ...
            @endphp
            <div class="progress-bar" style="width: {{ $progress }}%; background: linear-gradient(90deg, var(--accent), var(--purple));"></div>
        </div>
    </div>
</div>