...
@section('content')
    ...
    <div style="display:flex;gap:.5rem;flex-wrap:wrap;margin-bottom:1.25rem;">
        <a href="{{ route('finance.ledger.student-fees') }}" class="btn-fin btn-outline-fin btn-sm"><i class="fas fa-arrow-left"></i> Back to Search</a>
        <button class="btn-fin btn-outline-fin btn-sm" onclick="window.print()"><i class="fas fa-print"></i> Print Statement</button>
    </div>

    <div class="fin-card">
        <div class="fin-card-header">
            <h3><i class="fas fa-calendar"></i> Filter by Academic Year</h3>
        </div>
    ...
    <div class="stat-grid" style="grid-template-columns:repeat(3,1fr);">
        <div class="stat-card">
            <div class="value">UGX {{ number_format($totalCharges, 0) }}</div>
            <div class="label">Total Charged</div>
        </div>
        <div class="stat-card">
            <div class="value">UGX {{ number_format($totalPaid, 0) }}</div>
            <div class="label">Total Paid</div>
        </div>
        <div class="stat-card">
            <div class="value" style="color:{{ $arrears > 0 ? 'var(--fin-red)' : 'var(--fin-green)' }};">
                UGX {{ number_format($arrears, 0) }}
            </div>
            <div class="label">{{ $arrears > 0 ? 'Balance Due (Arrears)' : ($arrears < 0 ? 'Credit Balance' : 'Fully Paid') }}</div>
        </div>
    </div>
    ...
    <div class="fin-card">
        ...
        @if($statement->isEmpty())
            <div class="empty-state">
                ...
                <p>No fee charges or payments recorded for this student{{ $year ? " in $year" : '' }}.</p>
            </div>
        @else
                    ...
                    <tbody>
                        @foreach($statement as $line)
                            <tr>
                                ...
                                <td>
                                    <span class="badge-fin badge-{{ $line['type'] === 'charge' ? 'amber' : 'green' }}" style="margin-right:.5rem;">
                                        {{ $line['type'] === 'charge' ? 'Charge' : 'Payment' }}
                                    </span>
                                    {{ $line['description'] }}
                                </td>
                                <td class="amount-mono">{{ $line['debit'] ? 'UGX ' . number_format($line['debit'], 0) : '—' }}</td>
                                <td class="amount-mono" style="color:var(--fin-green);">{{ $line['credit'] ? 'UGX ' . number_format($line['credit'], 0) : '—' }}</td>
                                <td class="amount-mono">UGX {{ number_format($line['balance'], 0) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
