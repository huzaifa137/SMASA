...
@section('page-header')
    <div class="fin-hero mt-5">
        <div style="position:relative;z-index:1;">
            <div class="hero-badge"><i class="fas fa-list-ul"></i> Finance — Ledgers</div>
            <h1>General Ledger</h1>
            <p>Every transaction posted to an account, with a running balance</p>
        </div>
    </div>
@endsection
...
@section('content')
    ...
    <div style="display:flex;gap:.5rem;flex-wrap:wrap;margin-bottom:1.25rem;">
        ...
        <a href="{{ route('finance.ledger.general') }}" class="btn-fin btn-primary-fin btn-sm"><i class="fas fa-list-ul"></i> General Ledger</a>
    ...
    <div class="fin-card">
        <div class="fin-card-header">
            <h3><i class="fas fa-filter"></i> Select Account</h3>
        </div>
        <form method="GET" class="filters">
                ...
                <select name="account_id" required>
                    ...
                    @foreach($accounts as $acc)
                        <option value="{{ $acc->id }}" @selected(($selectedAccount->id ?? null) == $acc->id)>
                            {{ $acc->account_code }} — {{ $acc->name }} ({{ ucfirst($acc->type) }})
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="filter-actions">
                <button class="btn-fin btn-primary-fin"><i class="fas fa-eye"></i> View Ledger</button>
            </div>
        </form>
    </div>
    ...
    @if($selectedAccount)
        <div class="stat-grid" style="grid-template-columns:repeat(3,1fr);">
            ...
            <div class="stat-card">
                <div class="value">UGX {{ number_format($openingBalance, 0) }}</div>
                <div class="label">Opening Balance</div>
            </div>
            <div class="stat-card">
                <div class="value">UGX {{ number_format($closingBalance, 0) }}</div>
        ...
        <div class="fin-card">
            <div class="fin-card-header">
                <h3><i class="fas {{ $selectedAccount->typeIcon() }}"></i> {{ $selectedAccount->account_code }} — {{ $selectedAccount->name }}</h3>
                <span class="badge-fin badge-{{ $selectedAccount->typeBadge() }}">{{ ucfirst($selectedAccount->type) }} · Normal {{ $selectedAccount->normalBalance() }}</span>
            </div>

            @if($entries->isEmpty())
                <div class="empty-state">
                    <i class="fas fa-inbox"></i>
                    <p>No transactions posted to this account for the selected period.</p>
                </div>
            @else
                        ...
                        <tbody>
                            @foreach($entries as $entry)
                                <tr>
                                    ...
                                    <td class="amount-mono">{{ $entry->reference_number }}</td>
                                    <td>{{ $entry->description }}</td>
                                    <td>
                                        <span class="badge-fin badge-{{ $entry->type === 'refund' ? 'red' : ($entry->type === 'income' ? 'green' : 'amber') }}">
                                            {{ ucfirst($entry->type) }}
                                        </span>
                                    </td>
                                    <td class="amount-mono" style="color:{{ $entry->signed_amount < 0 ? 'var(--fin-red)' : 'var(--fin-green)' }};">
                                        {{ $entry->signed_amount < 0 ? '-' : '+' }}UGX {{ number_format(abs($entry->signed_amount), 0) }}
                                    </td>
                                    <td class="amount-mono">UGX {{ number_format($entry->running_balance, 0) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
    ...
    @else
        <div class="fin-card">
            <div class="empty-state">
                ...
                <p>Choose an account above to view its ledger.</p>
            </div>
        </div>
    @endif
        </div>
