{{-- resources/views/Finance/fee-categories.blade.php --}}
...
<style>
    ...
    * {
    ...
    .category-header { display: flex; align-items: center; gap: .75rem; margin-bottom: .75rem; }
    ...
    .category-name { font-weight: 700; color: var(--text-1); font-size: 1rem; margin-bottom: .25rem; display: flex; align-items: center; gap: .4rem; flex-wrap: wrap; }
    ...
    .stat-item { display: flex; align-items: center; gap: .3rem; color: var(--text-2); }
    .stat-value { font-weight: 700; color: var(--text-1); font-family: 'DM Mono', monospace; }
    ...
    .edit-cat { color: #2f2ccb; }
    .edit-cat:hover { background: rgba(47, 44, 203, .1); }
    .delete-cat { color: var(--fin-red); }
    .delete-cat:hover { background: var(--fin-red-l); }
    ...
    .modal-header h3 { margin: 0; font-size: 1.1rem; font-weight: 700; color: #fff; display: flex; align-items: center; gap: 0.5rem; }
    .close-modal { cursor: pointer; font-size: 1.5rem; color: rgba(255, 255, 255, 0.7); transition: all 0.2s; line-height: 1; }
    ...
    .modal-body::-webkit-scrollbar-thumb:hover { background: #2f2ccb; }
    ...
    .color-hex { font-family: 'DM Mono', monospace; font-size: .8rem; color: var(--text-2); }
    ...
    .icon-option i { font-size: 1rem; color: var(--text-2); }
    .icon-option:hover { background: rgba(47, 44, 203, .1); border-color: #2f2ccb; }
    .icon-option.selected { background: #2f2ccb; border-color: #2f2ccb; }
    .icon-option.selected i { color: #fff; }
    ...
    small { font-size: .7rem; color: var(--text-3); margin-top: .2rem; display: block; }

    @media(max-width:768px) {
        .fin-hero { padding: 1.5rem; }
        .fin-hero h1 { font-size: 1.3rem; }
        .stat-grid { grid-template-columns: 1fr; }
        .categories-grid { grid-template-columns: 1fr; padding: 1rem; }
        .modal-content { width: 95%; }
        .icon-selector { grid-template-columns: repeat(6, 1fr); }
        .action-buttons button { padding: .3rem .6rem; font-size: .7rem; }
    }
</style>
@endsection
...
@section('page-header')
    <div class="fin-hero mt-5">
        <div style="position:relative;z-index:1;">
            <div class="hero-badge"><i class="fas fa-tags"></i> Finance — Fee Categories</div>
            <h1>Fee Categories</h1>
            <p>The categories your school pays into — used on fee structures and when recording payments</p>
        </div>
    </div>
@endsection
...
@section('content')
    ...
    <div class="stat-grid">
        ...
        <div class="stat-card">
            <div class="value">{{ $categories->where('is_external', true)->count() }}</div>
            <div class="label">Non-Fee / External Categories</div>
        </div>
    </div>
    ...
    <div class="fin-card">
        <div class="fin-card-header">
            <h3><i class="fas fa-th-large"></i> Fee &amp; Payment Categories</h3>
            ...
            <div class="categories-grid">
                @foreach($categories as $cat)
                    <div class="category-card {{ $cat->is_active ? '' : 'inactive' }}">
                        <div class="category-header">
                            ...
                            <div class="category-info">
                                <div class="category-name">
                                    {{ $cat->name }}
                                    @if($cat->is_external)
                                        <span class="badge-fin badge-amber"><i class="fas fa-tools"></i> Non-Fee</span>
                                    @endif
                                    @if(!$cat->is_active)
                                        <span class="badge-fin badge-red">Inactive</span>
                                    @endif
                                </div>
                                <div class="category-stats">
                                    <div class="stat-item">
                                        <i class="fas fa-list-ul"></i>
                                        <span class="stat-value">{{ $cat->structure_items_count ?? 0 }}</span>
                                        <span>fee items</span>
                                    </div>
                                    <div class="stat-item">
                                        <i class="fas fa-receipt"></i>
                                        <span class="stat-value">{{ $cat->payment_items_count ?? 0 }}</span>
                                        <span>payments</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        @if($cat->description)
                            <div style="font-size:.75rem;color:var(--text-3);margin-bottom:.5rem;padding:.3rem 0;border-top:1px dashed var(--border);">
...
<div id="categoryModal" class="modal">
    <div class="modal-content">
        ...
        <form method="POST" action="{{ route('finance.fee-categories.store') }}" id="categoryForm">
            @csrf
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <input type="text" name="name" id="catName" required
                        placeholder="e.g., Tour, Transport, Damages">
                </div>
                ...
                <div class="form-group">
                    <label>Icon</label>
                    <input type="text" name="icon" id="catIcon" value="fa-tag" placeholder="fa-icon-name"
                    ...
                    <div class="icon-selector" id="iconSelector">
                        ...
                        <div class="icon-option" data-icon="fa-briefcase-medical"><i class="fas fa-briefcase-medical"></i></div>
                        <div class="icon-option" data-icon="fa-file-signature"><i class="fas fa-file-signature"></i></div>
                        ...
                        <div class="icon-option" data-icon="fa-shuttle-van"><i class="fas fa-shuttle-van"></i></div>
                        ...
                        <div class="icon-option" data-icon="fa-swimming-pool"><i class="fas fa-swimming-pool"></i></div>
                        ...
                        <div class="icon-option" data-icon="fa-id-card"><i class="fas fa-id-card"></i></div>
                        <div class="icon-option" data-icon="fa-window-maximize"><i class="fas fa-window-maximize"></i></div>
                        <div class="icon-option" data-icon="fa-exclamation-triangle"><i class="fas fa-exclamation-triangle"></i></div>
                        <div class="icon-option" data-icon="fa-ellipsis-h"><i class="fas fa-ellipsis-h"></i></div>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" id="catDesc" rows="2"
                        placeholder="What this category is for..."></textarea>
                </div>

                <div class="form-group">
                    <label class="checkbox-label">
                        <input type="checkbox" name="is_external" id="catExternal" value="1">
                        <span>Non-fee / external category</span>
                    </label>
                    <small>Use this for ad-hoc charges outside any fee structure — e.g. a broken item, a fine. It'll be suggested first when recording a non-fee payment.</small>
                </div>
                ...
                <div class="form-group" id="catActiveGroup" style="display:none;">
                    ...
                    <small>Inactive categories won't appear when building fee structures or recording payments</small>
                </div>
            </div>
    ...
    <script>
        ...
        const catExternal = document.getElementById('catExternal');
        const catActive = document.getElementById('catActive');
        const catActiveGroup = document.getElementById('catActiveGroup');
        ...
        document.querySelectorAll('.icon-option').forEach(icon => {
            icon.addEventListener('click', function () {
                catIcon.value = this.dataset.icon;
        ...
        function openAddModal() {
            ...
            catIcon.value = 'fa-tag';
            catDesc.value = '';
            catExternal.checked = false;
            catActive.checked = true;
            catActiveGroup.style.display = 'none';
            categoryForm.action = '{{ route('finance.fee-categories.store') }}';
        ...
        function openEditModal(id, name, color, icon, description, isExternal, isActive) {
            ...
            catExternal.checked = isExternal;
            catActive.checked = isActive;
            catActiveGroup.style.display = 'block';
            categoryForm.action = `{{ url('finance/fee-categories') }}/${id}`;
        ...
        function confirmDeleteCategory(id, name) {
            Swal.fire({
                ...
                html: `<span style="color:#475569;">Are you sure you want to delete <strong>${name}</strong>?<br><br>If it's already used on a fee structure or a payment, it'll be deactivated instead so past records keep their category.</span>`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    form.method = 'POST';
                    form.action = `{{ url('finance/fee-categories') }}/${id}`;
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Swal.fire({
                        ...
                        allowEscapeKey: false,
                        showConfirmButton: false,
                        didOpen: () => { Swal.showLoading(); categoryForm.submit(); }
                    });
                }
            });
        });
        ...
        @if(session('success'))
            Swal.fire({
                ...
                confirmButtonColor: '#2f2ccb', timer: 3000, timerProgressBar: true
            });
        @endif
