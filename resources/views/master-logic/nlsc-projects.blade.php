...
@section('content')
    <div class="side-app">
        <div class="nt-hero">
            <span class="hero-badge"><i class="fas fa-diagram-project me-1"></i> NLSC Projects</span>
            <div class="hero-title">Projects — {{ $seniorLabel }}</div>
            <div class="hero-subtitle">
                Manage the Project Work Project Areas, Projects and their Competency Areas for
                each Senior/Subject. Type these in from your own copy of the NCDC syllabus —
                nothing here is pre-loaded for you.
            </div>
        </div>
        ...
        <div class="nt-card nt-card-filters">
            <div class="card-body-custom" style="display:flex; flex-wrap:wrap; gap:1rem; align-items:flex-end;">
                <form method="GET" action="{{ route('admin.nlsc-projects') }}" id="filterForm"
                    style="display:flex; flex-wrap:wrap; gap:1rem; align-items:flex-end; flex:1;">
                    ...
                    <div style="min-width:220px;">
                        ...
                        <select id="assessmentTypeSelect" class="form-control">
                            <option
                                value="{{ route('admin.nlsc-topics', ['senior' => $selectedSenior, 'subject' => $selectedSubject]) }}">
                                Activities of Integration</option>
                            <option
                    ...
                    <div style="min-width:200px;">
                        ...
                        <select name="senior" class="form-control"
                            ...
                            @foreach ($seniorOptions as $opt)
                                <option value="{{ $opt->md_id }}" @if((int) $opt->md_id === (int) $selectedSenior) selected
                                @endif>{{ $opt->md_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div style="min-width:200px;">
                        ...
                        <select name="subject" id="subjectNativeSelect" class="d-none" tabindex="-1" aria-hidden="true">
                            @foreach ($subjectOptions as $opt)
                                <option value="{{ $opt->md_id }}" @if((int) $opt->md_id === (int) $selectedSubject) selected
                                @endif>{{ $opt->md_name }}</option>
                            @endforeach
                        </select>
                        ...
                        <div class="nt-select" id="subjectSelect" data-native="subjectNativeSelect">
                            <button type="button" class="nt-select-trigger" aria-haspopup="listbox"
                                aria-expanded="false">
                            ...
                            <div class="nt-select-panel" role="dialog">
                                <div class="nt-select-search">
                                    ...
                                    <input type="text" class="nt-select-search-input"
                                        placeholder="Search subject…" autocomplete="off">
                    ...
                    <tbody id="projectsTbody">
                        @forelse ($projects as $index => $project)
                            <tr data-id="{{ $project->id }}">
                                ...
                                <td><span class="nt-desc-preview description-cell"
                                        title="{{ $project->description }}">{{ $project->description ?: '—' }}</span></td>
                                <td>
                                ...
                                <td>
                                    <button type="button" class="nt-action-btn btn-view-sm view-project-btn"><i
                                            class="fas fa-eye"></i> View</button>
                                    <button type="button" class="nt-action-btn btn-edit-sm edit-project-btn"><i
                                            class="fas fa-pen"></i> Edit</button>
                                    <button type="button" class="nt-action-btn btn-del-sm delete-project-btn"><i
        ...
        <div class="nt-modal-box">
            ...
            <div class="nt-modal-body">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="projectAreaInput" class="form-control" list="existingAreasList"
                    ...
                    <small class="text-muted" id="projectAreaHelp">Type an existing Project Area to add to it, or a new name to create
                        one.</small>
                </div>
                <div class="form-group mt-2">
                    <label class="form-label">Project Name</label>
                    <input type="text" id="projectNameInput" class="form-control"
                        placeholder="e.g. School Communication Campaign">
                </div>
                <div class="form-group mt-2">
                    <label class="form-label">Description</label>
                    <textarea id="projectDescriptionInput" class="form-control" rows="3"
                        placeholder="e.g. Learners design a communication campaign addressing an issue affecting learners in their school."></textarea>
                </div>
            </div>
    ...
    <div class="nt-modal-overlay" id="viewProjectModal">
        <div class="nt-modal-box" style="max-width:640px;">
            <div class="nt-modal-hd">
                ...
                <button class="nt-modal-close" onclick="closeNtModal('viewProjectModal')"><i
    ...
    <div class="nt-modal-overlay" id="bulkImportModal">
        <div class="nt-modal-box" style="max-width:560px;">
            <div class="nt-modal-hd">
                <h4><i class="fas fa-file-upload me-2"></i> Bulk Import Projects</h4>
                <button class="nt-modal-close" onclick="closeNtModal('bulkImportModal')"><i
                        class="fas fa-times"></i></button>
            </div>
            <div class="nt-modal-body">
                ...
                <div
                    style="background:#f8f7ff; border:1.5px solid #e4e2ff; border-radius:.7rem; padding:.8rem 1rem; font-size:.78rem; margin-bottom:1rem;">
                    <code>senior</code> — e.g. "Senior 1"<br>
                    <code>subject</code> — e.g. "English"<br>
                    <code>project_area</code> — e.g. "Communication"<br>
                    <code>project</code> — the project name<br>
                    <code>description</code> — optional. The project's brief.<br>
                    <code>competency_area</code> — optional. Separate more than one with a
                    <strong> | </strong> (pipe) character.
                </div>
                <p style="font-size:.8rem; color:#8f8cae;">
                    Rows that repeat the same Senior/Subject/Project Area/Project reuse the
                    existing project rather than duplicating it — useful for adding more
                    competency areas to a project already on the list.
                </p>
            ...
            <div class="nt-modal-ft">
                ...
                <button class="btn btn-primary" id="submitBulkImportBtn"><i class="fas fa-upload me-1"></i> Upload &amp;
                    Import</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        (function () {
            ...
            const panel       = root.querySelector('.nt-select-panel');
        ...
        document.querySelectorAll('.edit-project-btn').forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                // below), matching the same "just use the Edit modal"
                // pattern on the school side.
                document.getElementById('projectAreaInput').disabled = false;
        ...
        document.getElementById('saveProjectBtn').addEventListener('click', function () {
            ...
            const isEdit = !!id;
            const url = isEdit
                ? `{{ url('admin/nlsc-projects') }}/${id}`
                : `{{ route('admin.nlsc-projects.store') }}`;
        ...
        function openViewProjectModal(projectId) {
            fetch(`{{ url('admin/nlsc-projects') }}/${projectId}/competency-areas`, {
        ...
        document.getElementById('submitBulkImportBtn').addEventListener('click', function () {
            ...
            fetch(`{{ route('admin.nlsc-projects.bulk-import') }}`, {
                method: 'POST',
                ...
                .then(res => {
                    ...
                    if (res.projects_imported > 0 || res.competency_areas_imported > 0) {
